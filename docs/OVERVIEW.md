...
今日の3件に空きがあるときは「＋ 今日やるものを選ぶ」からPickerを開き、次の一手・やりたいことを最大3件選びます。ドラッグ中だけ現れる専用Drop Zoneで今日の採用を外せますが、元の登録は残ります。未完了項目は翌日へ自動繰り越ししません。
...
ファイル、ショートカット、ブラウザーのURL、ごみ箱などをD&Dで登録できます。指定フォルダーへ設定・実行記録・アイコンキャッシュの日次ZIPバックアップを保存し、検証後にアプリ内から復元できます。設定の「メンテナンス」からソフトウェアリセットを実行でき、バックアップの要否と最終確認を順に選びます。実行中または一時停止中のTimerは先に終了します。登録先のファイル、手順書本体、既存バックアップは削除せず、内部snapshotとjournalで失敗時のrollbackと次回起動時の復旧を行い、成功後は初回状態で再起動します。
...
v1.3.0ではconfigをv3へ更新し、Project metadataとNextStep実行設定を分離しています。v2読み込み時は変換前raw backupを作成し、変換・全体検証・backupのいずれかが失敗した場合は元のconfigを書き換えません。空の旧NextStepに実行設定だけが残る場合は一度限りの保留設定として保存し、次の一手を設定するときに引き継ぐか破棄するかを選びます。同じProjectの置換前後を区別するoptional generation markerは新規設定から付け、既存Today3やSessionを遡及更新しません。
