...
- Builder見出しに新規登録の追加ボタンは置かない。右クリックまたはShift+F10では「次の一手から追加」「やりたいことから追加」を表示し、選択した登録元へsmooth scrollして見出しへfocusする。reduced motionでは即時移動する。
...
- 候補は「次の一手」「やりたいこと」にまとめる。Builder内の「やりたいこと」は同じProjectの候補をProject見出しの下へまとめ、見出しごとに開閉できる。件数はProject名の直後へ表示する。プロジェクト名は読み取れるmuted meta、行動文は本文相当の濃さと大きさにし、操作menuは常時表示する。Today3を選ぶcanonicalな画面はこのBuilderとする。
- 候補行のプロジェクト名・行動文の列位置とhover/focus背景は登録元の行と揃える。右端の「今日へ」操作は維持する。
- 採用済み候補はstable source identityで判定し、同じ文面のやりたいことも別項目として扱う。
- 候補はD&Dと右クリックの上下移動で並べ替えられる。drag中の自動スクロールで行位置が動いても、直前の有効な挿入位置を保持してdropを確定する。
...
- ボタンは高さ36px、短時間・通常は幅66px、計測は幅60pxとし、全体を200pxへ収めて3/2/1列を崩さない。計測中は経過時間・一時停止/再開・終了を同じ200px幅のcompact stripへ収める。
...
- NextStep cardのProject領域、本文領域、右上の「…」は同じcontext menuを開く。順序は「プロジェクトを編集」「プロジェクトを管理」、次の一手関連操作、区切り、「今日を組み立てるに登録する」とする。「やりたいことを追加」は混在させない。
...
- NextStep cardではProject操作とNextStep操作を1つのmenuへ統合する。Project自体の並べ替えは従来どおりD&Dで行える。
...
- 右クリックから次の一手にする、上へ移動、下へ移動、編集、完了にする、削除を行う。項目menuには「やりたいことを追加」を置かず、追加入口は見出しへ集約する。Wishlist自身には「今日へ」を置かず、Today3への採用はToday Builderから行う。完了は項目を一覧から外してsnapshotを作り、削除はsnapshotを作らない。どちらも確認を必須とする。
...
- 各項目は次の一手と同じcompact rowとし、行動文と右端の操作メニューを配置する。D&Dできることを示す専用記号は表示しない。未設定は「プロジェクトなし」と表示する。
