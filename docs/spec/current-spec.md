...
- 右側上部に「N分で始める」、通常開始の「通常 N分」、時間を決めない計測を横並びで表示する。説明表記は今やる一手だけに使い、Today3はcompactな「N分」表示を維持する。
...
- 候補は「次の一手」「やりたいこと」にまとめ、プロジェクト名を伴うcompact rowで表示する。Today3を選ぶcanonicalな画面はこのBuilderとする。
...
- 次の一手は、0〜5件では全件、6〜19件では先頭5件を表示する。6〜19件では「残りN件をもっと見る」で全件へ展開し、「5件だけ表示」で戻せる。
- 次の一手が20件以上では10件ずつページ表示する。やりたいことはプロジェクトグループごとに同じ件数規則を適用する。見出し件数と保存対象は常に全件で、展開・ページ移動だけではconfigを保存しない。Today Builderは従来どおり5件ずつ表示する。
- 追加、編集、削除、完了、並べ替え、19件と20件の境界変更後もstable IDを基準に対象のページまたは展開状態を選び、可能な限りフォーカスを維持する。最終ページがなくなった場合は有効なページへ戻す。
...
- ボタンは高さ36px、短時間・通常は幅約90px、計測はcompactな固定寸法とし、3桁分数でも3/2/1列を崩さない。
- 短時間は初期状態で「N分で始める」、通常は「通常 N分」を表示し、ホバーまたはkeyboard focusで左側の再生アイコンを重ねる。文言の中心位置と既存アニメーションは維持する。計測は時計アイコンを常時表示する。
...
- プロジェクトごとのグループをProject表示順で並べ、所属のない項目は最後の「未分類」にまとめる。各グループは独立して開閉状態、件数、ページ状態を持つ。
- D&Dと上下移動は同じプロジェクトグループ内だけで手動順序を変更し、ゴーストと黄色挿入線を表示する。別グループへのdropでは所属Projectを変更しない。
