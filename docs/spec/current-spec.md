...
- 行動文は左端から15px内側に置き、「今やる一手」のラベル文字開始位置に揃える。補足と開始ボタンの位置は変えない。
- 14日以上実行がない場合は短時間からの再開を案内する。
- 関連手順書がある場合だけ手順書ボタンを表示する。
- 右側上部に再生アイコン付きの「N分で始める」、通常開始の「通常 N分」、時間を決めない計測を横並びで表示する。再生アイコンは常時表示し、短時間・通常buttonのhover/focusでは下端accentだけを短く展開する。Today3はcompactな「N分」表示を維持する。
...
- 候補は「次の一手」と「やりたいこと」から構成し、勝利条件や過去の実行記録は候補に含めない。
- Today3が0件の場合は「今日やるものを選びましょう」と「次の一手・やりたいことから選べます」、`＋ 今日を組み立てる` CTAを実カード1行相当の領域に表示する。CTAはBuilderを展開して先頭候補または見出しへfocusする。
- Builder見出しに新規登録の追加操作は置かない。次の一手・やりたいことの登録は、それぞれの登録元見出しから行う。
...
- 候補は「次の一手」「やりたいこと」にまとめる。Builder内の「やりたいこと」は同じProjectの候補をProject見出しの下へまとめ、見出しごとに開閉できる。プロジェクト名は読み取れるmuted meta、行動文は本文相当の濃さと大きさにし、操作menuは常時表示する。Today3を選ぶcanonicalな画面はこのBuilderとする。
...
- Builder候補をToday3へD&Dすると、3/2/1列の実カード矩形から求めた位置へ採用する。Today3へ直接送れるD&D元はBuilderだけとする。
- 当日明示的に除外した次の一手・やりたいことだけをBuilderのバーまたは展開領域へD&Dして候補へ入れられる。誘導は「ここにドロップして今日を組み立てるに入れる」とし、Today3へは自動採用せず、既に候補のsourceでは復帰先を強調しない。
...
- 次の一手はProjectを高さ約120pxのcompact cardとして、広幅3列・中幅2列・狭幅1列で表示する。左上のProject名と本文はToday3と同じ文字階層にし、操作menuを右上、設定・変更操作を右下へ揃え、D&D順序変更と右クリック操作を維持する。未設定本文は「まだ次の一手がありません」とする。
- 次の一手は初期6件まで表示し、7件以上では `＋ 残りN件を表示` で全件を展開し、`− 折りたたむ` で6件へ戻す。専用ページ送りは設けず、展開だけではconfigを保存しない。
- やりたいことはプロジェクトグループごとに既存の件数規則を適用する。見出し件数と保存対象は常に全件で、展開・ページ移動だけではconfigを保存しない。Today Builderは従来どおり5件ずつ表示する。
...
- 見出しや本文に新規登録の追加ボタンを置かない。0件時は「今日やるものを選びましょう」と「今日を組み立てる」を表示し、Builderを展開して先頭候補または見出しへ移動・フォーカスする。
...
- 最終行に重複した罫線を表示しない。
- カード左下のghost actionと右クリックの「今日の3件から外す」で、対象のToday3採用だけを解除する。確認ダイアログは出さず、保存成功後に「今日の3件から外しました」と通知し、8秒間「元に戻す」を実行できる。
- カードの右クリックと「…」は同じ操作メニューを開く。キーボードではShift+F10、メニュー内の矢印キー、Escapeとフォーカス復帰を利用できる。
...
- ボタンは高さ36px、短時間・通常は幅78px、計測は時計アイコンだけの34px固定寸法とし、3桁分数でも3/2/1列を崩さない。計測中は経過時間・一時停止/再開・終了を同じ200px幅のcompact stripへ収める。
- 短時間と通常は初期状態でcompactな「N分」を表示し、ホバーまたはkeyboard focusで左側の再生アイコンを重ねる。文言の中心位置と既存アニメーションは維持する。計測は時計アイコンを常時表示する。
...
- 「今日の3件から外す」のUndoは対象項目だけを元の前後関係へ戻す。「今日の候補から外す」のUndoは当日の除外を解除し、その操作でToday3も外した場合だけ同じ採用snapshotを戻す。
- Undoは最新configへ対象単位の逆差分を適用し、後から行った別項目の並べ替え、設定変更、Session、source、完了履歴を上書きしない。
...
- Project領域とNextStep領域は別々のpointer/focus/context targetとする。Projectの右クリックは「プロジェクトを編集」「やりたいことを追加」「プロジェクト管理」、設定済みNextStepの右クリックは「次の一手を編集」「次の一手を変更」「次の一手を未設定にする」「今日を組み立てるに登録する」を提供する。
...
- 今日の3件への採用は「今日を組み立てる」の「今日へ」を主経路とする。現在NextStepの`generationId`をToday snapshotの`sourceGenerationId`へcopyし、元のプロジェクトを残す。
...
- NextStep領域の右クリックには「完了にする」「削除」を置かない。「次の一手を未設定にする」は確認後に「やりたいことへ戻す」「削除して未設定にする」「キャンセル」から選ぶ。どちらの未設定化もProject、採用済みToday3 snapshot、Session、source completionを変更しない。
...
- 見出し補足は「他にやりたいこと。今日やるものは『今日を組み立てる』から選べます。」。
...
- 右クリックから次の一手にする、上へ移動、下へ移動、編集、完了にする、削除を行う。Wishlist自身には「今日へ」を置かず、Today3への採用はToday Builderから行う。完了は項目を一覧から外してsnapshotを作り、削除はsnapshotを作らない。どちらも確認を必須とする。
...
- 両セクションの見出しは名称・件数・説明・右端操作の順とし、右端に黄色の追加ボタンと「…」メニューを置く。やりたいことの一覧に装飾の左線は表示しない。
- 今日を組み立てる・やりたいことの行は最小48px、上下余白5pxを基準にする。Builderの候補見出しは本文と競合しないreadable muted metaとする。
...
- Today Builder候補を登録元の「次の一手」または「やりたいこと」へD&Dすると、元sourceを残したまま当日の候補から外す。drag開始直後から対応する登録元セクションを黄色で誘導し、drop時は既存の候補除外保存処理を使う。
...
- セッションnoteを分類・タグ化しない。
- 実行記録の追加、編集、削除、復元、ホットリロード後に再計算する。
- 件数上限は設けない。
...
- 計測は0:00から実効経過時間を増やし、予定時間、満了、OS満了通知、15分延長、進捗率を持たない。一時停止時間は他のTimerと同じく除外する。
- 短時間・通常・計測の相互切替では、現在の1本を同じ終了処理で確定してから選んだmodeを開始する。切替前が1分未満なら記録しない。
- 満了後は15分の延長導線を持つ。
- noteは自由テキストのまま保存する。
- 通知は右下の共通Toastで表示する。通常通知は4秒、Undo操作付きは表示開始から8秒を標準とする。
- pointerがToast内、focusがToast内、または文書が非表示のいずれかでは残り時間を停止し、全条件が解けた時点から再開する。待ち行列の時間はUndo猶予へ含めない。
- 同時表示は最大3件とし、それを超える未処理Undoは待ち行列に保持する。新しいToastがfocusを奪うことはなく、reduced-motionでは入退場と残時間railの動きを抑える。
...
- 短時間・通常は残り時間と進捗を表示し、計測は「計測中」と実効経過時間を表示する。計測に架空の予定時間や進捗率を作らない。
...
- active pageは表示対象を決める状態で、背景と下端の金色lineで示す。外周の黄色outlineは使わない。
- keyboard focusはactive pageとは独立し、ページまたは項目へ黄色outlineで示す。項目からページへ移ると項目の選択表示を消すが、最後の項目はページ別に記憶する。
- ページ上でDownを押すと、そのページで最後にfocusした有効な項目へ戻る。記憶した項目が削除済みなら先頭の有効な項目へfallbackする。
- 固定ページは並べ替え対象外、ユーザーページはD&Dで左右へ移動できる。
- ラベル、グループ、ページ名、aliases、説明を検索対象にする。最近使ったもの、おすすめ、使用回数順は持たない。
- 登録数にかかわらずタイル寸法を共通化する。長いラベルは省略し、titleで全文を補完する。
- タイルは同一ページ内と別ページ間でD&D移動できる。ghostはpointerから約10pxずらし、別ページtabへ一定時間hoverするとページを切り替える。
- タイルはLeft / Rightで同じvisual row、Up / Downで最寄りのvisual rowへfocusを移す。
- 初回表示は最初のタイルへfocusする。閉じるとactive page、focus層、ページ別の最後の項目、scroll位置を記憶し、再表示時に復元する。
- 再表示時の検索語は復元せず空に戻す。検索結果は一時状態として扱う。
- 辞書windowがfocus中かつ入力・tab・button操作中でなければ、余白からもfocus中タイルの矢印移動を続けられる。
- 検索入力中の矢印キーと、メイン画面・他アプリのキー操作は奪わない。文字入力は検索欄へ送る。
- 最上段タイルのUpはactive pageへ戻り、categoryのLeft / Rightはactive pageを変えずfocusだけを移す。
- categoryはEnter / Spaceでactiveにし、Downでそのページの項目へ移動する。
- mouse hoverはkeyboard focusや記憶中の項目を書き換えない。
...
記録画面は役割を混ぜず、次の3タブに分ける。初期表示は「ふりかえり」。明示的な `← メイン` でメイン画面へ戻る。
...
- `src/components/HelpGuideDialog.tsx`
...
- `tests/visual/phase8-today3-builder.spec.ts`
- `tests/visual/phase8-records.spec.ts`
- `tests/visual/phase8-dictionary-state.spec.ts`
- `tests/visual/phase61-guide.spec.ts`
- `docs/v1.3.0-ui-ux-spec.md`
- `docs/ui-audit/UI-UX-MASTER-AUDIT.md`
- Phase 8の監査・作業報告書

計画書や過去の記憶ではなく、Phase 8候補の実装と自動テストを優先して記述している。
