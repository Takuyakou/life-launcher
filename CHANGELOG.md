...
- Drop Registerは内部action prefixと不要な登録前Toastを隠し、登録先groupと表示先を意図的に選べる構成へ整理した。
- アプリ内Guideを、最初の3分と毎日の流れを中心にした初心者向け10章へ書き直した。
- ユーザー向け表記を「手順書ビューアー」へ統一した。
- 手順書ビューアーへ3段階のウィンドウサイズ切替と、ドラッグ・キーボードで調整できる可変サイドバーを追加した。
...
- Timer調整時の文字選択を抑止し、次の一手カードのhover枠をProject色へ揃えた。
