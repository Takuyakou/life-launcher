...
test("Guide states the current registration, selection, and completion flow", async ({ page }) => {
  ...
  await expect(today).toContainText("その日の分を完了にするか選べます");
  await expect(today).toContainText("採用後にプロジェクトの設定を変えても、今日の基準は変わりません");
  ...
  await expect(timer).toContainText("初期フォーカスは右です");
  await expect(timer).toContainText("どちらも実行記録は残ります");
